...
public interface TossPaySwagger {
        ...
        ResponseEntity<ApiResponse<TossReadyResponse>> pickupPaymentRequest(@RequestBody PaymentRequest paymentRequest);
        ...
        ResponseEntity<ApiResponse<TossApproveResponse>> tossPaymentConfirm(
                        @RequestParam("paymentKey") String paymentKey,
                        @RequestParam("orderId") String orderId,
                        @RequestParam("amount") int amount);

        @Operation(summary = "결제 실패 콜백", description = "결제 진행 중 실패했을 때 호출되는 콜백 URL입니다.")
        ResponseEntity<ApiResponse<Void>> fail(@RequestParam("orderId") String orderId);
